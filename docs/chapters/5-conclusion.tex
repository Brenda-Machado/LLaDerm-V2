...
Neste trabalho, foi realizado o \textit{fine-tuning} do \ac{MLLM} \ac{LLaMA}-3.2 com as técnicas de treinamento
...
Os resultados apresentados constataram que o \textit{fine-tuning} do modelo \ac{LLaMA}-3.2 pode ser feite de forma
eficiente através do treinamento com \ac{QLoRA}. Os modelos treinados com este método obtiveram um desempenho
semelhante ao dos modelos treinados com \ac{LoRA}.

Os resultados obtidos na etapa inicial foram satisfatórios e demonstraram que através do \textit{fine-tuning} com
\ac{QLoRA} utilizando 5000 pares de imagem e resposta de classificação é possível atingir uma acurácia na classificação
de lesões de pele de 85,4\%. Porém, os resultados da etapa final foram poucos satisfatórios quanto ao critério de
classificação de lesões, atingindo apenas 45,2\% de acurácia após o treinamento com \ac{QLoRA}. Embora o volume de
dados utilizado nesta etapa tenha sido similar, com um total de 4736 pares de imagem e texto de resposta, o conteúdo em
sí das imagens e do texto eram consideravelmente diferentes. As imagens de aproximação utilizadas possuíam uma
complexidade visual muito maior, pois elas incluíam elementos extras além da lesão de pele, como o resto do ambiente ao
redor do local da lesão. Além disso, os textos utilizados possuíam não só uma seção para a classificação da lesão, mas
também uma seção para a classificação de risco, conclusão sobre a lesão e conclusão geral.
...
Por fim, os resultados dos testes na segunda etapa sobre o conjunto de treinamento evidenciaram que o \ac{LLaMA}-3.2
após o \textit{fine-tuning} não foi capaz de capturar o padrão de classificação de lesões de pele, sugerindo um
\textit{underfitting} neste aspecto.

Em geral, considera-se que seria necessária a adaptação e expansão do conjunto de dados para que o desenvolvimento de
um modelo com um maior desempenho seja possível.
...
tornaria possível uma comparação mais avançada entre diferentes modelos.
